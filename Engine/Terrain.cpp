...
bool Terrain::InitializeBuffers(ID3D11Device * device )
{
	...
	for (j = 0; j<(m_terrainHeight - 1); j++)
	{
		for (i = 0; i<(m_terrainWidth - 1); i++)
		{
			...
			/*triangle.vertex[0] = DirectX::SimpleMath::Vector3(m_heightMap[index3].x, m_heightMap[index3].y, m_heightMap[index3].z);
			triangle.vertex[1] = DirectX::SimpleMath::Vector3(m_heightMap[index4].x, m_heightMap[index4].y, m_heightMap[index4].z);
			triangle.vertex[2] = DirectX::SimpleMath::Vector3(m_heightMap[index1].x, m_heightMap[index1].y, m_heightMap[index1].z);

			square.triangles[0] = triangle;


			triangle.vertex[0] = DirectX::SimpleMath::Vector3(m_heightMap[index1].x, m_heightMap[index1].y, m_heightMap[index1].z);
			triangle.vertex[1] = DirectX::SimpleMath::Vector3(m_heightMap[index4].x, m_heightMap[index4].y, m_heightMap[index4].z);
			triangle.vertex[2] = DirectX::SimpleMath::Vector3(m_heightMap[index2].x, m_heightMap[index2].y, m_heightMap[index2].z);

			square.triangles[1] = triangle;
...
bool Terrain::pointDistance(DirectX::SimpleMath::Vector3 A, DirectX::SimpleMath::Vector3 B)
{
	if (sqrt(pow(B.x - A.x, 2) + pow(B.y - A.y, 2) + pow(B.z - A.z, 2)) < 3) 
	{
		return false;
	}
	return true;
...
void Terrain::UpdateChanges(ID3D11Device* device)
{
...
bool Terrain::GenerateWaterMap(ID3D11Device* device)
{
	bool result;
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			index = (m_terrainHeight * j) + i;
			double noise = m_noiseMultiplier * perlinNoise.Noise((i+m_offset)*m_scale,(j+m_offset)*m_scale,1);
			m_heightMap[index].x = (float)i;
	...
	for (int j = 0; j < m_terrainHeight; j++)
	{
		...
		{
			index = (m_terrainHeight * j) + i;
			if (i == 0 || i == m_terrainWidth || j == m_terrainHeight || j == 0) 
			{
				continue;
			}

			if (m_heightMap[index].y > m_heightMap[index + 1].y
				&& m_heightMap[index].y > m_heightMap[index - 1].y
				&& m_heightMap[index].y > m_heightMap[index + m_terrainWidth].y
				&& m_heightMap[index].y > m_heightMap[index - m_terrainWidth].y
				&& m_heightMap[index].y > m_heightMap[index + m_terrainWidth + 1].y
				&& m_heightMap[index].y > m_heightMap[index + m_terrainWidth - 1].y
				&& m_heightMap[index].y > m_heightMap[index - m_terrainWidth + 1].y
				&& m_heightMap[index].y > m_heightMap[index - m_terrainWidth - 1].y) 
			{
				highPoints.push_back(SimpleMath::Vector3(m_heightMap[index].x, m_heightMap[index].y, m_heightMap[index].z));
			}
		}
	...
	//the water terrain will be generated in the vertex shader
	/*result = InitializeBuffers(device);
	if (!result)
	{
		return false;
...
bool Terrain::GenerateGroundMap(ID3D11Device* device)
{
	bool result;
	...
	for (int j = 0; j < m_terrainHeight; j++)
	{
		for (int i = 0; i < m_terrainWidth; i++)
		{
			...
			index = (m_terrainHeight * j) + i;
			double noise = m_noiseMultiplier * perlinNoise.Noise(i * m_scale, j * m_scale, 1);
			m_heightMap[index].x = (float)i;
			m_heightMap[index].y = noise;
			m_heightMap[index].z = (float)j;
